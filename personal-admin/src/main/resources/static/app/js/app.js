...
App.run(["$rootScope", "$state", "$stateParams", '$window', '$templateCache', '$cookieStore', '$http', '$filter',
    function ($rootScope, $state, $stateParams, $window, $templateCache, $cookieStore, $http, $filter) {
        ...
        $rootScope.ImgHostUrl = "http://img.xsmore.com/wxrobot";
        ...
        //------地区信息联动
        //加载所有地区信息
        // $http.post('/common/allareas', {}).success(function (response) {
        //     $rootScope.allAreas = response;
        // });
        //地区信息初始化
        $rootScope.initArea = function (provinceName, cityName) {
            $rootScope.Provinces = [];
            $rootScope.Cities = [];
            $rootScope.Areas = [];
            var city = $filter('filter')($rootScope.allAreas, function (item) {
                return item.Name == cityName;
            });
            var province = $filter('filter')($rootScope.allAreas, function (item) {
                return item.Name == provinceName;
            });
            $rootScope.allAreas.forEach(function (e) {
                if (e.ParentId == 0) {
                    $rootScope.Provinces.push(e);
                }
                if (province && province.length > 0 && e.ParentId == province[0].AreaId) {
                    $rootScope.Cities.push(e);
                }
                if (city && city.length > 0 && e.ParentId == city[0].AreaId) {
                    $rootScope.Areas.push(e);
                }
            });
        };
        //省信息改变，获取市信息
        $rootScope.getCities = function (provinceName) {
            var province = $filter('filter')($rootScope.allAreas, function (item) {
                return item.Name == provinceName;
            });
            $rootScope.Cities = $filter('filter')($rootScope.allAreas, function (item) {
                return item.ParentId == province[0].AreaId;
            });
            $rootScope.Areas = [];
        };
        //市信息改变,获取区信息
        $rootScope.getAreas = function (cityName) {
            var city = $filter('filter')($rootScope.allAreas, function (item) {
                return item.Name == cityName;
            });
            $rootScope.Areas = $filter('filter')($rootScope.allAreas, function (item) {
                return item.ParentId == city[0].AreaId;
            });
        };

        //获取所有物流信息
        // $http.post('/common/allexpress', {}).success(function (response) {
        //     $rootScope.allExpress = response;
        // });

        //枚举转换
        $rootScope.convertToEnum = function (value, enumList) {
            var result = "未设置";
        ...
        $rootScope.GetJsonKey = function (json) {
            for (var key in json) {
        ...
        if (angular.isDefined(cuser) && !angular.isDefined($rootScope.user)) {
            // $http.post('/EmployeeInfo/', { Id: cuser.Id }).then(function (response) {
            //     $rootScope.user = {
            //         userauthname: response.data.UserAuthName,
            //         id: response.data.UserAuthId,
            //         username: response.data.UserName,
            //         displayname: response.data.DisplayName,
            //         role: response.data.Roles[0],
            //         departmentid: response.data.Company,
...
App
            ...
            return {
                responseError: function (rejection) {
                    ...
                    if (rejection.status === 401) {
                        console.log($state);
                        //console.log($state.current.name !== "page.login");
                        if ($state != undefined && $state.current.name !== "page.login") {
                            $state.go("page.login", {redirect: "#" + $location.url()});
                        } else {
    ...
    .service('loginModal', function ($modal, $cookieStore, $location, $state, $http, $rootScope) {
        function assignCurrentUser(user) {
            ...
            // $http.get('/EmployeeInfo/' + user.Id).then(function (response) {
            //     $rootScope.user = {
            //         userauthname: response.data.UserAuthName,
            //         id: response.data.UserAuthId,
            //         username: response.data.UserName,
    ...
    .constant('APP_REQUIRES', {
        ...
        modules: [
            ...
            },
            {
                name: 'summernote',
                files: [
                    'vendor/summernote/bootstrap.min.js',
                    'vendor/summernote/summernote.js',
                    'vendor/summernote/summernote.css',
                    //   'vendor/summernote/lang/summernote-zh-CN.js',
                    'vendor/summernote/angular-summernote.min.js'
                ]
            },
            ...
            {
                name: 'lmlSample', files: ['css/lmlSample.css']
            },
            {name: 'ngTableExport', files: ['vendor/ng-table-export/ng-table-export.js']},
            {name: 'ngcsv', files: ['vendor/ng-csv/ng-csv.js']},
            {
            ...
            },
            {
                name: 'ngJsTree',
                files: [
                    'vendor/ngJsTree/angular-busy.css',
                    'vendor/ngJsTree/angular-busy.js',
                    'vendor/ngJsTree/ngJsTree.js',
                    'vendor/ngJsTree/style.min.css',
                    'vendor/ngJsTree/underscore-min.js'
                ]
            },
            {
                name: 'jsTree',
                files: [
                    'vendor/ngJsTree/jstree.js',
                ]
            },
            {
                name: 'htmlSortable',
            ...
            },
            {
                name: 'datetimepicker',
                files: [
                    'vendor/datetimepicker/datetime-picker.js',
                    'vendor/datetimepicker/ui-bootstrap-tpls.js'
                ]
            },
            {
            ...
            {
                name: 'agentapplycss',
                files: ['http://cdn.xsmore.com/CSS/Common/base.css?v=160302', 'http://cdn.xsmore.com/CSS/zmkm/agency-accredit.css']
            }
        ]
    });
...
App.controller('LoginFormController', ['$scope', '$http', '$state', '$stateParams', '$rootScope', '$cookieStore', '$window', 'Notify',
    function ($scope, $http, $state, $stateParams, $rootScope, $cookieStore, $window, Notify) {
            ...
            if ($scope.loginForm.$valid) {
                ...
                $http({
                    ...
                    if (response.status == 200) {
                        ...
                        Notify.alert("登录失败，账号或密码错误", {status: 'warning'});
                    }
                    $scope.loginText = "登录";
                }, function (x) {
...
App.service('ngTableDataService', ['$filter', function ($filter) {
    ...
    var TableData = {
        ...
        getServerSideParams: function (orderBy, p, requestParams) {

            var s = $filter('limitTo')(orderBy, 1, 0);
            var sort = orderBy.replace("-", "").replace("+", "");
            if (s === "-") {
                requestParams.OrderByDesc = sort;
            } else {
...
App.controller('AppController',
    ['$rootScope', '$scope', '$state', '$translate', '$window', '$localStorage', '$timeout', 'toggleStateService', 'colors', 'browser', 'cfpLoadingBar', 'loginModal', '$cookieStore', '$http',
        function ($rootScope, $scope, $state, $translate, $window, $localStorage, $timeout, toggle, colors, browser, cfpLoadingBar, loginModal, $cookieStore, $http) {
            "use strict";
...
(function ($, window, document) {
    ...
    var Message = function (options) {
        ...
        if (!containers[this.options.pos]) {
            containers[this.options.pos] = $('<div class="uk-notify uk-notify-' + this.options.pos + '" style="z-index:99999;"></div>').appendTo('body').on("click", ".uk-notify-message", function () {
...
/**=========================================================
 * Module: helpers.js
 * Provides utils functions for routes definition
 =========================================================*/
